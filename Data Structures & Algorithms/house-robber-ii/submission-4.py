class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        #------------------------------------
        # ✅ Bottom-Up DP iteratively with Tabulation (dp list)
        #   Time : O(n) — each house processed once per subproblem
        #   Space: O(n) — full DP table for each case
        ...
        def helper(arr):
            dp = [0] * len(arr)             # dp[i] = max money robbed from house 0 to i
            dp[0] = arr[0]                  # base case: rob first house
            dp[1] = max(arr[0], arr[1])     # rob max of first vs. second house

            for i in range(2, len(arr)):
                dp[i] = max(dp[i - 1], dp[i - 2] + arr[i])
                ''' Option 1: skip current → dp[i - 1]
                    Option 2: rob current  → dp[i - 2] + arr[i] '''
