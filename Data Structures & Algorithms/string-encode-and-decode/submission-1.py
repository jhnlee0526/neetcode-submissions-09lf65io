class Solution:
    # TIME COMPLEXITY: O(n + L) where n is the number of strings, and L is the total length of all strings combined.
    # SPACE COMPLEXITY: O(L) for the resulting encoded string.
    def encode(self, strs: List[str]) -> str:
        return "".join(f'{len(s)}#{s}' for s in strs)
        # 4#neet4#code4#love3#you

    # TIME COMPLEXITY: O(L) where L is the total length of the encoded string.
    # SPACE COMPLEXITY: O(L) for storing the decoded strings.
